...
async fn main() -> anyhow::Result<()> {
    ...
    worker_system
        .register_handler("analyze_trade", |_job| {
            async move {
        ...
        while let Some(update) = whale_watcher.next().await {
            match update {
                ...
                | aurora_db::reactive::updates::QueryUpdate::Modified { new: doc, .. } => {
                    ...
                    println!(
                        "  [ WHALE ALERT] High-value trade detected: {} worth ${:.2}",
                        symbol, total_value
                    );
                }
                _ => {}
    ...
    for i in 0..10 {
        ...
        for j in 0..batch_size {
            ...
            trade.insert(
                "side".to_string(),
                Value::String(if id % 2 == 0 { "BUY" } else { "SELL" }.to_string()),
            );
    ...
    println!(
        "Simulation finished in {:?} ({:.0} trades/sec)",
        duration,
        50000.0 / duration.as_secs_f64()
    );
