...
async fn main() -> anyhow::Result<()> {
    ...
    worker_system
        .register_handler("dispatch_repair", |job| async move {
            let drone_id = job.payload.get("drone_id").unwrap().as_str().unwrap();
            println!(
                "  [REPAIR] Dispatching repair technician to Drone: {}",
                drone_id
            );
            Ok(())
        })
    ...
    tokio::spawn(async move {
        while let Ok(event) = dashboard_stream.recv().await {
            ...
            if matches!(
                event.change_type,
                aurora_db::pubsub::events::ChangeType::Insert
            ) {
        ...
        while let Some(update) = safety_watcher.next().await {
            match update {
                aurora_db::reactive::updates::QueryUpdate::Added(doc) => {
                    if let (
                        Some(aurora_db::Value::String(drone_id)),
                        Some(aurora_db::Value::Float(temp)),
                    ) = (doc.data.get("drone_id"), doc.data.get("temp"))
                    {
                        println!(
                            "  [ALARM] Critical Heat! Drone {} is at {:.1}C",
                            drone_id, temp
                        );
                    }
                }
    ...
    for i in 0..(num_heartbeats / batch_size) {
        ...
        for j in 0..batch_size {
            ...
            heartbeat.insert(
                "status".to_string(),
                Value::String(if temp > 90.0 { "CRITICAL" } else { "OK" }.to_string()),
            );
        ...
        for j in 0..batch_size {
            ...
            if temp > 98.0 {
                ...
                worker_system
                    .enqueue(Job::new("dispatch_repair").with_payload(payload))
                    .await?;
            }
        }
    }
    ...
    println!(
        "Throughput: {:.0} heartbeats/sec",
        num_heartbeats as f64 / duration.as_secs_f64()
    );
    ...
    if let aurora_db::parser::executor::ExecutionResult::Query(res) = result {
        ...
        for edge in edges {
            ...
            println!(
                "  - {} | Temp: {:.1}C",
                node.get("drone_id").unwrap(),
                node.get("temp").unwrap().as_f64().unwrap()
            );
        }

        let pi = conn.data.get("pageInfo").unwrap().as_object().unwrap();
        println!(
            "\n  Next Incident Page Available: {}",
            pi.get("hasNextPage").unwrap().as_bool().unwrap()
        );
    }
