...
async fn test_rich_query_operators() {
    ...
    db.new_collection(
        ...
        vec![
            (
                "name",
                FieldType::Scalar(aurora_db::types::ScalarType::String),
                false,
            ),
            (
                "role",
            ...
            (
                "age",
                FieldType::Scalar(aurora_db::types::ScalarType::Int),
                false,
            ),
        ],
    )
    ...
    let returned_names: Vec<_> = results
        .iter()
        .filter_map(|d| {
            if let Some(Value::String(s)) = d.data.get("name") {
                Some(s.clone())
            } else {
                None
            }
    ...
    assert_eq!(
        results[0].data.get("name").unwrap(),
