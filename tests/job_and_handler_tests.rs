...
fn fd(field_type: FieldType) -> FieldDefinition {
    FieldDefinition { field_type, nullable: true, unique: false, indexed: false, ..Default::default() }
}
...
async fn test_enqueue_job_via_aql_returns_metadata() {
    ...
    if let ExecutionResult::Mutation(res) = result {
        ...
        assert_eq!(doc.data.get("job_type"), Some(&Value::String("send_email".into())));
        assert_eq!(doc.data.get("status"), Some(&Value::String("pending".into())));
...
async fn test_enqueue_job_payload_available_in_handler() {
    ...
    let received_to: Arc<std::sync::Mutex<Option<String>>> =
    ...
    db.workers
        ...
        .register_handler("send_email", move |job: Job| {
            ...
            async move {
                let to = job.payload.get("to").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    let received = wait_until(
        || received_to.lock().unwrap().is_some(),
        2000,
    )
    .await;
...
async fn test_define_handler_on_insert_fires() {
    ...
    db.new_collection("audit_log", vec![
    ...
    if let ExecutionResult::Query(r) = res {
        assert_eq!(r.documents[0].data.get("status"), Some(&Value::String("registered".into())));
    } else {
    ...
    let audit_written = wait_until(
        || {
            db_ref
                .scan_and_filter("audit_log", |doc| {
                    doc.data.get("user_id") == Some(&Value::String(uid.clone()))
                }, None)
                .map(|v| !v.is_empty())
                .unwrap_or(false)
        },
    ...
    assert!(audit_written, "audit_log should have a row for the new user");

    let audit_docs = db
        .scan_and_filter("audit_log", |_| true, None)
        .unwrap();
...
async fn test_define_handler_on_update_fires() {
    ...
    db.new_collection("price_history", vec![
    ...
    let logged = wait_until(
        || {
            db_ref
                .scan_and_filter("price_history", |doc| {
                    doc.data.get("product_id") == Some(&Value::String(pid.clone()))
                }, None)
                .map(|v| !v.is_empty())
                .unwrap_or(false)
        },
...
async fn test_define_handler_on_delete_fires() {
    ...
    db.new_collection("sessions", vec![
        ("user_id", fd(FieldType::SCALAR_STRING)),
    ...
    db.new_collection("revoked_tokens", vec![
        ("session_id", fd(FieldType::SCALAR_STRING)),
    ])
    .await
    .unwrap();
    ...
    let revoked = wait_until(
        || {
            db_ref
                .scan_and_filter("revoked_tokens", |doc| {
                    doc.data.get("session_id") == Some(&Value::String(sid.clone()))
                }, None)
                .map(|v| !v.is_empty())
                .unwrap_or(false)
        },
    ...
    assert!(revoked, "revoked_tokens should contain the deleted session id");
}
...
async fn test_define_handler_field_variables_propagated() {
    ...
    db.new_collection("notifications", vec![
        ("customer_id", fd(FieldType::SCALAR_STRING)),
    ...
    let notified = wait_until(
        || {
            db_ref
                .scan_and_filter("notifications", |doc| {
                    doc.data.get("customer_id") == Some(&Value::String("cust-99".into()))
                }, None)
                .map(|v| !v.is_empty())
                .unwrap_or(false)
        },
    ...
    assert!(notified, "notification should contain customer_id from the inserted order");

    let notifs = db.scan_and_filter("notifications", |_| true, None).unwrap();
    assert_eq!(notifs[0].data.get("message"), Some(&Value::String("order_placed".into())));
}
...
async fn test_upsert_inserts_when_not_found() {
    ...
    assert_eq!(docs[0].data.get("value"), Some(&Value::String("dark".into())));
}
...
async fn test_transaction_executes_all_ops() {
    ...
    .await
    .unwrap();
    db.new_collection("tx_log", vec![
        ("note", fd(FieldType::SCALAR_STRING)),
    ])
    .await
    .unwrap();
    ...
    assert_eq!(logs[0].data.get("note"), Some(&Value::String("initial_funding".into())));
}
