...
async fn test_high_contention_scenario() {
    ...
    for i in 0..num_hot_keys {
        ...
        let count = all_docs.iter()
            .filter(|doc| {
                matches!(doc.data.get("key_id"), Some(Value::String(k)) if k == key_clone.as_str())
            })
            .count();
    ...
    println!("SUCCESS: High contention handled correctly");
}
...
async fn test_mixed_workload_stress() {
    println!("\n=== Mixed Workload Stress Test (OPTIMIZED) ===");
    ...
    println!("SUCCESS: Mixed workload completed successfully");
}
