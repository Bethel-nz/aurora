...
async fn test_transaction_rollback_works() {
    ...
    let doc_id = aurora_db::transaction::ACTIVE_TRANSACTION_ID
        .scope(tx_id, async {
            db.insert_into(
                "users",
                vec![
    ...
    assert!(
        doc.is_none(),
        "Document should not exist in storage before commit"
    );
    ...
    assert!(
        doc.is_none(),
        "Document should have been removed (or never committed) after rollback"
    );
}
...
async fn test_aql_transaction_rollback_works() {
    ...
    let alice = db
        .query("accounts")
        .filter(|f: &aurora_db::query::FilterBuilder| f.eq("name", "Alice"))
        .collect()
        .await
        .unwrap();
    assert_eq!(
        alice[0].data.get("balance").unwrap(),
        &Value::Int(100),
        "Balance should be 100 after rollback"
    );
...
async fn test_transaction_commit_works() {
    ...
    let doc_id = aurora_db::transaction::ACTIVE_TRANSACTION_ID
        .scope(tx_id, async {
            db.insert_into("users", vec![("name", Value::String("Bob".to_string()))])
                .await
                .unwrap()
        })
        .await;
    ...
    assert!(doc.is_some());
    assert_eq!(
        doc.unwrap().data.get("name").unwrap(),
